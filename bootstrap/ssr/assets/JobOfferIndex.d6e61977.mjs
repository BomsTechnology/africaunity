import { onMounted, ref, resolveComponent, mergeProps, withCtx, unref, createVNode, openBlock, createBlock, createTextVNode, toDisplayString, useSSRContext } from "vue";
...
const _sfc_main = {
  __name: "JobOfferIndex",
  __ssrInlineRender: true,
  setup(__props) {
    const { jobOffers, getJobOffers, destroyJobOffer, loading, errors } = useJobOffers();
    onMounted(async () => {
      await getJobOffers();
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative h-auto w-full xl:mt-0 xl:p-4" }, _attrs))}><div class="z-0 h-full w-full p-4"><div class="flex justify-between bg-white px-8 py-5 shadow-lg"><h1 class="text-4xl font-bold capitalize text-primary-blue"> JobOffer </h1>`);
      _push(ssrRenderComponent(_component_router_link, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<p class="text-base leading-4"${_scopeId}>Add JobOffer</p>`);
          } else {
            return [
              createVNode(unref(PlusCircleIcon), { class: "h-6 w-6" }),
              createVNode("p", { class: "text-base leading-4" }, "Add JobOffer")
            ];
          }
        }),
        _: 1
      ...
      _push(ssrRenderComponent(_component_EasyDataTable, {
        ...
        "item-company_logo": withCtx((item, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="py-2"${_scopeId}>`);
            if (item.company_logo) {
              _push2(`<img${ssrRenderAttr("src", item.company_logo)} alt="" class="h-12 w-12 rounded-full object-cover"${_scopeId}>`);
            } else {
              _push2(ssrRenderComponent(unref(OfficeBuildingIcon), { class: "h-12 w-12" }, null, _parent2, _scopeId));
            }
            _push2(`</div>`);
          } else {
            return [
              createVNode("div", { class: "py-2" }, [
                item.company_logo ? (openBlock(), createBlock("img", {
                  key: 0,
        ...
        "item-min_price": withCtx((item, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(item.min_price)} ${ssrInterpolate(item.currency.symbol)} - ${ssrInterpolate(item.max_price)} ${ssrInterpolate(item.currency.symbol)}`);
          } else {
            return [
              createTextVNode(toDisplayString(item.min_price) + " " + toDisplayString(item.currency.symbol) + " - " + toDisplayString(item.max_price) + " " + toDisplayString(item.currency.symbol), 1)
            ];
          }
        }),
          ...
          if (_push2) {
            ...
            return [
              createVNode("div", null, [
                ...
                createVNode("button", {
                  type: "button",
                  onClick: ($event) => deleteJobOffer(item.id),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/views/back/jobOffer/JobOfferIndex.vue");
