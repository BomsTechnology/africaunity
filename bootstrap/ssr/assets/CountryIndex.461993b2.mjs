...
const _sfc_main = {
  ...
  setup(__props) {
    const { countries, getCountries, destroyCountry, loading, errors } = useCountries();
    ref("");
    onMounted(async () => {
      await getCountries();
    });
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative h-auto w-full xl:mt-0 xl:p-4" }, _attrs))}><div class="z-0 h-full w-full p-4"><div class="flex justify-between bg-white px-8 py-5 shadow-lg"><h1 class="text-4xl font-bold text-primary-blue">Country</h1>`);
      _push(ssrRenderComponent(_component_router_link, {
        to: { name: "admin.country.create" },
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<p class="text-base leading-4"${_scopeId}>Add Country</p>`);
          } else {
            return [
              createVNode(unref(PlusCircleIcon), { class: "h-6 w-6" }),
              createVNode("p", { class: "text-base leading-4" }, "Add Country")
            ];
          }
        }),
        _: 1
          ...
          if (_push2) {
            ...
            return [
              createVNode("div", null, [
                ...
                createVNode("button", {
                  onClick: ($event) => deleteCountry(item.id),
                  type: "button",
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/views/back/country/CountryIndex.vue");
