import { ref, onMounted, withAsyncContext, reactive, mergeProps, unref, withCtx, createTextVNode, toDisplayString, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    onMounted(
      ...
      ([__temp, __restore] = withAsyncContext(() => getContinents()), __temp = await __temp, __restore(), __temp),
      ([__temp, __restore] = withAsyncContext(() => getZones()), __temp = await __temp, __restore(), __temp),
      ([__temp, __restore] = withAsyncContext(() => getCountries()), __temp = await __temp, __restore(), __temp),
      ([__temp, __restore] = withAsyncContext(() => getMinistries()), __temp = await __temp, __restore(), __temp)
    );
    const post = reactive({
      title: "",
      type: props.type,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative h-auto w-full xl:mt-0 xl:p-4" }, _attrs))}><div class="z-0 w-full p-4"><div class="flex w-full justify-between bg-white px-8 py-5 shadow-lg"><h1 class="text-4xl font-bold capitalize text-primary-blue"> Add ${ssrInterpolate(__props.type)}</h1></div><section class="mx-auto w-full bg-white p-6 shadow-md">`);
      if (unref(errors) != "") {
      ...
      if (__props.type == "article") {
        _push(`<div class="col-span-2"><label class="dark:text-gray-200 text-gray-700" for="fr">Image</label><input required type="file" class="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:focus:border-blue-300 mt-2 block w-full rounded-md border border-gray-200 bg-white px-4 py-2 text-gray-700 focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40"></div>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<div class="col-span-2"><label class="dark:text-gray-200 text-gray-700" for="pt">Content</label><textarea required type="text" id="pt" class="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:focus:border-blue-300 mt-2 block h-32 w-full rounded-md border border-gray-200 bg-white px-4 py-2 text-gray-700 focus:border-primary-blue focus:outline-none focus:ring focus:ring-primary-blue focus:ring-opacity-40">${ssrInterpolate(post.content)}</textarea></div></div><div class="mt-6 flex justify-end">`);
      if (unref(loading) == 0) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/views/back/posts/PostCreate.vue");
