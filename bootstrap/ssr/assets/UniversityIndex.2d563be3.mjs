import { ref, onMounted, resolveComponent, mergeProps, withCtx, unref, createVNode, openBlock, createBlock, createCommentVNode, createTextVNode, toDisplayString, useSSRContext } from "vue";
...
const _sfc_main = {
  __name: "UniversityIndex",
  __ssrInlineRender: true,
  setup(__props) {
    const { universities, getAllUniversities, destroyUniversity, loading, errors } = useUniversities();
    ref("");
    onMounted(async () => {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative h-auto w-full xl:mt-0 xl:p-4" }, _attrs))}><div class="z-0 h-full w-full p-4"><div class="flex justify-between bg-white px-8 py-5 shadow-lg"><h1 class="text-4xl font-bold capitalize text-primary-blue"> University </h1>`);
      _push(ssrRenderComponent(_component_router_link, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            _push2(`<p class="text-base leading-4"${_scopeId}>Add University</p>`);
          } else {
            return [
              createVNode(unref(PlusCircleIcon), { class: "h-6 w-6" }),
              createVNode("p", { class: "text-base leading-4" }, "Add University")
            ];
          }
        }),
        _: 1
      ...
      _push(ssrRenderComponent(_component_EasyDataTable, {
        ...
        "item-name": withCtx((item, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(item.name.length <= 30 ? item.name : item.name.substring(0, 29) + "...")}`);
          } else {
            return [
              createTextVNode(toDisplayString(item.name.length <= 30 ? item.name : item.name.substring(0, 29) + "..."), 1)
            ];
          }
        }),
        "item-description": withCtx((item, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`${ssrInterpolate(item.description <= 30 ? item.description : item.description.substring(0, 29) + "...")}`);
          } else {
            return [
              createTextVNode(toDisplayString(item.description <= 30 ? item.description : item.description.substring(0, 29) + "..."), 1)
            ];
          }
        }),
          ...
          if (_push2) {
            ...
            return [
              createVNode("div", null, [
                ...
                createVNode("button", {
                  type: "button",
                  onClick: ($event) => deleteUniversity(item.id),
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/views/back/university/UniversityIndex.vue");
