...
import "vue-router";
const _sfc_main = {
  __name: "CategoryAnnouncementCreate",
  __ssrInlineRender: true,
  setup(__props) {
    const categoryAnnouncement = reactive({
      name: ""
    });
    const { createCategoryAnnouncement, errors, loading } = useCategoryAnnouncements();
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "relative h-auto w-full xl:mt-0 xl:p-4" }, _attrs))}><div class="z-0 h-full w-full p-4"><div class="flex justify-between bg-white px-8 py-5 shadow-lg"><h1 class="text-4xl font-bold text-primary-blue"> Add category Announcement </h1></div><section class="mx-auto bg-white p-6 shadow-md">`);
      if (unref(errors) != "") {
      ...
      _push(`<h2 class="text-md font-light text-gray-700"> Add a new category Announcement </h2><form><div class="mt-4 grid grid-cols-1 gap-6 sm:grid-cols-2"><div><label class="dark:text-gray-200 text-gray-700" for="fr">French</label><input id="fr"${ssrRenderAttr("value", categoryAnnouncement.name)} type="text" class="dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 dark:focus:border-blue-300 mt-2 block w-full rounded-md border border-gray-200 bg-white px-4 py-2 text-gray-700 focus:border-blue-400 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-40"></div></div><div class="mt-6 flex justify-end">`);
      if (unref(loading) == 0) {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/views/back/category_announcement/CategoryAnnouncementCreate.vue");
