...
import { u as useBusinessSizes, a as useBusinessTypes } from "./businessSizeServices.7658bab2.mjs";
  ...
  setup(__props) {
    ...
    const handleScroll = async (e) => {
      if (userContainer.value) {
        ...
        if (element.getBoundingClientRect().bottom < window.innerHeight && toGet.value && !isAll.value && (filter.residence_country == "" && filter.activity_area == "" && filter.business_size == "" && filter.business_type == "")) {
          toGet.value = false;
          page.value++;
          await getUsersType("business");
          toGet.value = true;
    ...
    const usersFilter = async () => {
      if (filter.residence_country != "" || filter.activity_area != "" || filter.business_size != "" || filter.business_type != "") {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<!--]--></select></div><div class="lg:text-sm text-xs"><label class="text-gray-700 dark:text-gray-200">${ssrInterpolate(_ctx.$t("activity-area"))}</label><select class="form-select block w-full px-4 py-2 mt-1 text-gray-700 bg-white border border-gray-200 rounded-md focus:outline-none focus:ring-primary-blue focus:border-primary-blue"><option value="">--------------</option><!--[-->`);
        ...
        ssrRenderList(unref(filteredUser), (user) => {
          ...
          if (showDetail.id == user.id && showDetail.state == true) {
            ...
            if (user.detail.residence_country) {
              _push(`<p class="leading-3"><h1 class="font-semibold text-sm">${ssrInterpolate(_ctx.$t("social-country"))}</h1><h2 class="font-light">`);
              if (_ctx.$i18n.locale == "en") {
            ...
            if (user.detail.business_type_id) {
              _push(`<p class="leading-3"><h1 class="font-semibold text-sm">${ssrInterpolate(_ctx.$t("type-company"))}</h1><h2 class="font-light">`);
              if (_ctx.$i18n.locale == "en") {
            ...
            if (user.detail.business_size_id) {
              _push(`<p class="leading-3"><h1 class="font-semibold text-sm">${ssrInterpolate(_ctx.$t("size-company"))}</h1><h2 class="font-light">`);
              if (_ctx.$i18n.locale == "en") {
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/views/front/Establishment.vue");
