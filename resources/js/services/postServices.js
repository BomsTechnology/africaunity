...
export default function usePosts() {
    ...
    //Get the minimal details of the post for the unauthentificated user when we share the post on social media
    const getPostShared = async (id) => {
        errors.value = "";
        try {
            loading.value = true;
            let response = await axios.get("/api/post-shared/" + id, {
                headers: {
                    Authorization: `Bearer ${localStorage.token}`,
                },
            });
