    ...
    <div class="relative min-h-screen w-full bg-white xl:mt-0 xl:p-4">
        <div class="z-0 h-full w-full p-4">
            <div class="flex justify-between bg-white px-8 py-5">
                <h1 class="text-4xl font-bold text-primary-blue">Event Mode</h1>
            </div>
...
const saveItem = async () => {
    loading.value = 1;
    if (isEdit.value) {
        await updateDemonstrationMode({ ...item }, id.value);
        id.value = 0;
        isEdit.value = false;
    } else {
        await createDemonstrationMode({ ...item });
    }
    await getDemonstrationModes();
    loading.value = 0;
    item.name_fr = "";
...
const deleteItem = async (id) => {
    if (confirm("I you Sure ?")) {
        loading.value = 1;
        await destroyDemonstrationMode(id);
        await getDemonstrationModes();
        loading.value = 0;
    }
};
...
const filteredItem = computed(() => {
    return demonstrationModes.value.filter((item) => {
        return item.name_en
            .toLowerCase()
