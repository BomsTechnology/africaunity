...
const saveItem = async () => {
    loading.value = 1;
    if (isEdit.value) {
        await updateDemonstrationNiche({ ...item }, id.value);
        isEdit.value = false;
        id.value = 0;
    } else {
        await createDemonstrationNiche({ ...item });
    }
...
const deleteItem = async (id) => {
    if (confirm("I you Sure ?")) {
        loading.value = 1;
        await destroyDemonstrationNiche(id);
        await getDemonstrationNiches();
        loading.value = 0;
    }
};
...
const filteredItem = computed(() => {
    return demonstrationNiches.value.filter((item) => {
        return item.name_en
            .toLowerCase()
