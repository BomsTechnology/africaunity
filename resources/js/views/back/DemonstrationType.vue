    ...
    <div class="relative min-h-screen w-full bg-white xl:mt-0 xl:p-4">
        <div class="z-0 h-full w-full p-4">
            <div class="flex justify-between bg-white px-8 py-5">
                <h1 class="text-4xl font-bold text-primary-blue">Event Type</h1>
            </div>
                ...
                <div class="bg-white px-6 pt-8 lg:w-[40%]">
                    <div>
                        <h1 class="mb-4 text-2xl font-semibold">
                            ...
                            <span v-else class="text-primary-blue underline"
                                >Edit
                            </span>
...
const saveItem = async () => {
    loading.value = 1;
    if (isEdit.value) {
        await updateDemonstrationType({ ...item }, id.value);
        id.value = 0;
        isEdit.value = false;
    } else {
        await createDemonstrationType({ ...item });
    }
    await getDemonstrationTypes();
    loading.value = 0;
    item.name_fr = "";
...
const deleteItem = async (id) => {
    if (confirm("I you Sure ?")) {
        loading.value = 1;
        await destroyDemonstrationType(id);
        await getDemonstrationTypes();
        loading.value = 0;
    }
};
...
const filteredItem = computed(() => {
    return demonstrationTypes.value.filter((item) => {
        return item.name_en
            .toLowerCase()
