...
<div class="flex">
  ...
  <div class="w-full xl:mt-0 mt-[74px] relative h-auto xl:p-4">
    ...
    <section class=" p-6 mx-auto bg-white shadow-md">
        ...
        <h2 class="text-md font-light text-gray-700">Edit a categoryAnnouncement</h2>
        <form @submit.prevent="saveCategoryAnnouncement()">
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                <div>
                    <label class="text-gray-700 dark:text-gray-200" for="fr">French</label>
                    <input id="fr" v-model="categoryAnnouncement.name" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                </div>
            </div>
...
export default {
    ...
    setup(props) {
        const { updateCategoryAnnouncement, getCategoryAnnouncement , errors, loading, categoryAnnouncement } = useCategoryAnnouncements();
        onMounted(getCategoryAnnouncement(props.id));     

        const saveCategoryAnnouncement = async () => {
            await updateCategoryAnnouncement(props.id);
        }
