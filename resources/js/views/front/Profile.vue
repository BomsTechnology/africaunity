                ...
                <div class="xl:w-[75%] lg:w-[65%] w-full px-8 py-2  h-full overflow-y-auto lg:mt-0 mt-20">
                    ...
                    <div v-else >
                        ...
                        <div class="space-x-2 flex items-center lg:py-0 py-1" v-if="user.id == loginUser.id">
                            ...
                            <router-link :to="{name:'setting.account',  params: {name: user.firstname, id : user.id }}">
    ...
    computed: {
        ...
        filteredPropAu() {
            return this.propau.filter((prop) => {
                if(this.langProp != ""){
                    return prop.title.toLowerCase().includes(this.searchProp.toLowerCase()) && prop.language == this.langProp;
                }else{
                    return prop.title.toLowerCase().includes(this.searchProp.toLowerCase());
                }
            });
        },
        filteredArticles() {
            return this.articles.filter((article) => {
                if(this.langArticle != ""){
                    return article.title.toLowerCase().includes(this.searchArticle.toLowerCase()) && article.language == this.langArticle;
                }else{
                    return article.title.toLowerCase().includes(this.searchArticle.toLowerCase());
                }
            });
        },
    },
