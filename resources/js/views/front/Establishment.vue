...
<div class=" text-xs lg:text-sm">
    <label class="text-gray-700 opacity-0 ">{{ $t('search') }} </label>
                    <button
                        type="button"
                        @click="usersFilter()"
                        class="w-full rounded-md bg-primary-blue mt-1 px-4 py-2.5 text-white"
                    >
                        {{ $t("search") }}
                    </button>
                </div>
...
onMounted(async () => {window.addEventListener("scroll", handleScroll); await getUsersType("business"); await getBusinessSizes(); await getCountries(); await getBusinessTypes(); await getActivityAreas();});
onUnmounted(async () => {
    page.value = 1;
